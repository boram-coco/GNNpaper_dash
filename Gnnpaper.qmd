...
def plot_fraud_over_time(cc_num=4.503100e+18):
    ...
    # Fraudulent Transactions (is_fraud = 1)의 색상을 빨간색으로 지정
    plt.scatter(data[data['is_fraud'] == 1]['trans_date_trans_time'], data[data['is_fraud'] == 1]['is_fraud'], marker='.', color='red', label='Fraudulent Transactions')
    
    # Legitimate Transactions (is_fraud = 0)의 색상을 파란색으로 지정
    plt.scatter(data[data['is_fraud'] == 0]['trans_date_trans_time'], data[data['is_fraud'] == 0]['is_fraud'], marker='.', color='blue', label='Legitimate Transactions')
    ...
    plt.legend(loc='center right')  # 범례를 오른쪽 가운데로 이동
    plt.show()
